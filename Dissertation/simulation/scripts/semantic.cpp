    ...
    if (cv_ptr_aux){
        ...
        for (int x = x_i; x < x_f; x++) {
            for (int y = y_i; y < y_f; y++) {
                cout << "VALOR DOS INDICES: [" << x << " , " << y << "] : " << cv_ptr_aux->image.at<float>(x,y) << endl;
                depth = cv_ptr_aux->image.at<float>(x,y);
                if (!isnan(depth)) {
...
void check_object_position(int object_pos_x, int object_pos_y, int cell_value){
    float odom_x, odom_y;
    odom_x = object_pos_x;
    odom_y = object_pos_y;
    // copy_map();
            ...
            if (object_pos_x < 640 && object_pos_y < 480) {
                ...
                if (grid_map_.info.height > 0) {
                    if (cv_ptr_){
                        ...
                        tie(screen_position_depth_x,screen_position_depth_y) = findSomeDepthValue(object_pos_x,msg->bounding_boxes[x].xmax,object_pos_y,msg->bounding_boxes[x].ymax,cv_ptr_);
                        if (screen_position_depth_x != -1 && screen_position_depth_y != -1) {
                            ...
                            float x_o = depth * ((screen_position_depth_x - cx)/fx);
                            float y_o = depth * ((screen_position_depth_y - cy)/fy);
                            cout << "INTRINSICOS: CX: " << cx << " | CY: " << cy << " | FX: " << fx << " | FY: " << fy << endl;
                            if (!isnan(x_o)  && !isnan(y_o)) {
                                cout << "POSICAO SUITCASE: [" << depth << " , " << -x_o << "]" << endl;
                                cout << "POSICAO SUITCASE EM RELACAO ROBO: [" << ROBOT_POSE_[0]+depth << " , " << ROBOT_POSE_[1]+x_o << "]" <<endl; 
                                check_object_position(depth,-x_o, SUITCASE_VALUE);
                            }
                        }
                    }
                }
                ...
                if (grid_map_.info.height > 0) {
                    if (cv_ptr_){
                        ...
                        if (screen_position_depth_x != -1 && screen_position_depth_y != -1) {
                            ...
                            cout << "INTRINSICOS: CX: " << cx << " | CY: " << cy << " | FX: " << fx << " | FY: " << fy << endl;
                            if (!isnan(x_o)  && !isnan(y_o)) {
                                cout << "POSICAO PERSON: [" << depth << " , " << x_o << "]" << endl;
                                cout << "POSICAO PERSON EM RELACAO ROBO: [" << ROBOT_POSE_[0]+depth << " , " << ROBOT_POSE_[1]+x_o << "]" <<endl; 
                                check_object_position(depth,-x_o, PERSON_VALUE);
                            }
                        }
                    }
                }
...
void depth_img_callback(const sensor_msgs::ImageConstPtr& depth_msg){    
...
int main(int argc, char **argv){
    ...
    ros::Subscriber depth_img = node.subscribe("/husky1/realsense/depth/image_rect_raw", 1, depth_img_callback);
